package br.com.digix.casasPopulares.application;
...
public class SimulaAplicacaoTest {
	
	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");

	@Test
	public void simulaAplicacaoTest() throws ParseException {
		...
		List<FamiliaDTO> ordenadoPorPontuacao = 
				FamiliaAptaCasaPopularCalculadora.getInstance().calcularPontuacao(
						FamiliaBuilder.getInstance().construirFamilia(pessoas)
				);
		
		assertTrue(ordenadoPorPontuacao.get(0).getSobrenome().equals("Carrara"));
		assertTrue(ordenadoPorPontuacao.get(1).getSobrenome().equals("Silva"));
		
		assertTrue(ordenadoPorPontuacao.get(0).getPontuacao().equals(2));
		assertTrue(ordenadoPorPontuacao.get(1).getPontuacao().equals(0));
